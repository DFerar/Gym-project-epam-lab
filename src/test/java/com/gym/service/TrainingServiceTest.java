...
public class TrainingServiceTest {
    ...
    public void shouldCreateTraining() {
        ...
        when(customerRepository.findCustomerEntityByGymUserEntityUserName(anyString())).thenReturn(customerEntity);
        when(instructorRepository.findInstructorEntityByGymUserEntityUserName(anyString())).thenReturn(
            instructorEntity);
        when(trainingTypeRepository.findByTrainingTypeName(any())).thenReturn(trainingTypeEntity);
        when(trainingRepository.save(trainingEntity)).thenReturn(trainingEntity);
        //When
        trainingService.createTraining(trainingEntity);
        //Assert
        verify(customerRepository, times(1)).findCustomerEntityByGymUserEntityUserName(anyString());
        verify(instructorRepository, times(1)).findInstructorEntityByGymUserEntityUserName(anyString());
    ...
    public void shouldGetCustomerListOfTrainings() {
        ...
        when(trainingRepository.findTrainingsByCustomerAndCriteria(customerUserName, fromDate, toDate, instructorName,
            trainingTypeName)).thenReturn(expectedTrainings);
        //When
        List<TrainingEntity> actualTrainings =
            trainingService.getCustomerListOfTrainings(customerUserName, fromDate, toDate, instructorName,
                trainingTypeName);
        //Assert
    ...
    public void shouldGetInstructorListOfTrainings() {
        ...
        when(trainingRepository.findTrainingsByInstructorAndCriteria(instructorUserName, fromDate, toDate,
            customerName)).thenReturn(expectedTrainings);

        List<TrainingEntity> actualTrainings =
            trainingService.getInstructorListOfTrainings(instructorUserName, fromDate, toDate, customerName);
